...
USING (
    SELECT 
        try_cast(restaurantid AS number) AS restaurant_id,  -- try_cast if fails to cast will return null -- but is column doesn't accept null -- it will not insert
        try_cast(name AS string) AS name,
        try_cast(cuisinetype AS string) AS cuisine_type,
        try_cast(pricing_for_2 AS number(10, 2)) AS pricing_for_two,
        try_cast(restaurant_phone AS string) AS restaurant_phone,
...
Initial LOAD -- 1st time Load

Delta Load -- subsequent Loads

Batch Processing -- when data processed daily

Micro-Batching -- when data processed every hour / 15 minutes.

Real-Tiem Straeming -- when every change is pushed to DWH via CDC.
...
list @stage_sch.csv_stg/delta/restaurant/;  -- csv_stg/delta/restaurant/RESTAU~1.CSV
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp() as _copy_data_ts
     from @stage_sch.csv_stg/delta/restaurant/RESTAU~1.CSV t
)
file_format = (format_name = 'stage_sch.csv_file_format')
on_error = abort_statement;
...
-- Similarily load day_2 restuant file data
-- foollow same steps
