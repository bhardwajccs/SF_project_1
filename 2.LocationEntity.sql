...
-- See data from Staged Loaction -- w some audiot columns that helps in Debugging.
SELECT 
...
-- Stage >>> Table
-- Make a Locations Table -- in Staging Schema
...
-- Stream Object on Location table -- Stream Object captures types of Operations on main table
-- Via Stream Objects we push data from one layer to another -- not main tables.
...
-- Move Data from Staging >>>> Clean Layer 
...
-- Clean Schema >>> Consumption Schema 
-- Clean Stream >>> dimLOCATION Table
...
-- Part-2 -- DELTA Operations

LIST @STAGE_SCH.CSV_STG/delta/location/;
-- Pick -- csv_stg/delta/location/delta-day01-2rows-add.csv

-- See data before Loading
Select * from stage_sch.location;
Select * from stage_sch.location_stm;


-- Only Copy above selected file
copy into stage_sch.location (locationid, city, state, zipcode, activeflag, 
                    createddate, modifieddate, _stg_file_name, 
                    _stg_file_load_ts, _stg_file_md5, _copy_data_ts)
from (
...
-- Move the delta Data from Staging table >>> Clean_Stg

-- RUN
-- ove Data from Staging >>>> Clean Layer  [Line 119 -- MERGE ST.]

Select * from RESTAURANT_LOCATION;



-- Check befroe Loading
Select * from clean_sch.restaurant_location_stm; -- Got 2 new records


-- Load from Clean_STG >>> dimLocation
-- Load dimLocation Script                    -- 2 records Loaded
...
-- Part - Loading the DELTA Data

list @STAGE_SCH.CSV_STG/delta/location;  -- delta-day02-2rows-update.csv



-- Only Copy above selected file
    --  RUN same COPY INTO [STAGE >>> stage_sch.location]
            ...
            from (
                select 
                    ...
                    metadata$file_content_key as _stg_file_md5,
                    current_timestamp as _copy_data_ts
                from @stage_sch.csv_stg/delta/location/delta-day02-2rows-update.csv t     -- delta-day02-2rows-update.csv
            )
            file_format = (format_name = 'stage_sch.csv_file_format')
            on_error = abort_statement;
    ...
    -- Now MERGE St. -- into clean_sch.restaurant_location  -- from  stage_sch.location_stm
...
Select * from RESTAURANT_LOCATION_STM; -- Will show all new data -- to be Loaded into dimLocation
...
-- Check SCD Type2
Select * from CONSUMPTION_SCH.RESTAURANT_LOCATION_DIM WHERE city = 'Mumbai';



-- USE CASE

-- What if Bad Records hit Source System will the Pipline RUN Like CSV File had Comman Delimiter but new data has Pipe Delimiter
-- Raw_Schema Table all data is Loaded into one column only
-- MERGE Will FAIL to load data to Cleaned Stage.
-- how to resove it ????


-- Take Invalid Data File
list @STAGE_SCH.CSV_STG/delta/location; -- delta-day03-invalid-delimiter.csv

-- Move this Invalid Data to Stage Table
            ...
            copy into stage_sch.location (locationid, city, state, zipcode, activeflag, 
                        ...
                        from (
                            select 
                                ...
                                metadata$file_content_key as _stg_file_md5,
                                current_timestamp as _copy_data_ts
                            from @stage_sch.csv_stg/delta/location/delta-day03-invalid-delimiter.csv t     -- delta-day02-2rows-update.csv
                        )
                        file_format = (format_name = 'stage_sch.csv_file_format')
                        on_error = abort_statement;
...
-- All Data loaded into first column only.
Select * from stage_sch.location;
Select * from stage_sch.location_stm;
...
-- Stream still has Data -- It will not be cleared.

Select * from stage_sch.location_stm;   -- Think How you will handle this Issue of wrong data


-- let's clear this Stream else it will keep failing
-- This will Clear Stream
CREATE TEMP TABLE common.location_tmp as 
    Select * from stage_sch.location_stm;    


-- Location Table has still Dirty Rcords -- DELETE those Reocrds
...
-- Similarily -- 4th File Dirty Data

list @STAGE_SCH.CSV_STG/delta/location;  --  delta-day04-invalid-files.csv
            ...
            copy into stage_sch.location (locationid, city, state, zipcode, activeflag, 
                        ...
                        from (
                            select 
                                ...
                                metadata$file_content_key as _stg_file_md5,
                                current_timestamp as _copy_data_ts
                            from @stage_sch.csv_stg/delta/location/delta-day04-invalid-files.csv t     -- delta-day02-2rows-update.csv
                        )
                        file_format = (format_name = 'stage_sch.csv_file_format')
                        on_error = abort_statement;
